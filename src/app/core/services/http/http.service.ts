                ...
                switch (status) {
                    ...
                    case NamedHttpStatus.BadRequest: {
                        if (event.message === 'VALIDATION_ERROR' && event.data && Array.isArray(event.data)) {
                            const errorMessages = event.data.map((error) => this.localize.translate.instant('VALIDATION.' + error)).join(', ');
                    ...
                    case NamedHttpStatus.InternalServerError: {
                        this.alertService.error(event.message ? this.localize.translate.instant('VALIDATION.' + event.message) : this.localize.translate.instant('VALIDATION.INTERNAL_SERVER_ERROR'));
                        break;
                    }
